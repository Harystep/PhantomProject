...
-(void)sendData{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSInteger i=1;
        NSInteger count = [BLImageHelper shareImageHelper].phassetChoosedArr.count;
        for (PHAsset *asset in [BLImageHelper shareImageHelper].phassetChoosedArr) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.progressBlock) {
                    self.progressBlock(i/(CGFloat)count);
                }
            });
            i++;
        }
...
-(void)exitImagePickerLib:(NSArray*)selectAry assetArrt:(NSArray*)assetArry editedImage:(UIImage *)image{
    ...
    if (self.presentingViewController) {
        [self dismissViewControllerAnimated:isCancel completion:nil];
    } else {
